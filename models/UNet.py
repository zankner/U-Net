...
class UNet(Model):
  def __init__(self, filters=16):
    ...
    for i in range(4):
      ds_step = []
      ds_step.append(Conv2D((2**i)*filters, 3, padding='same', kernel_regularizer=l1_l2(0.4, 0.4)))
      ds_step.append(BatchNormalization())
      ds_step.append(Conv2D((2**i)*filters, 3, padding='same', kernel_regularizer=l1_l2(0.4, 0.4)))
      ds_step.append(BatchNormalization())
      ds_step.append(MaxPool2D())
      self.encoder.append(ds_step)
    self.encoder_bot_0 = Conv2D(filters*16, 3, padding='same', kernel_regularizer=l1_l2(0.4, 0.4))
    self.encoder_bot_1 = Conv2D(filters*16, 3, padding='same', kernel_regularizer=l1_l2(0.4, 0.4))
    ...
    for i in range(4):
      up_step = []
      up_step.append(Conv2DTranspose((2**(3-i))*16, 3, 2, padding='same', kernel_regularizer=l1_l2(0.4, 0.4)))
      up_step.append(Conv2D((2**(3-i))*16, 3, padding='same', kernel_regularizer=l1_l2(0.4, 0.4)))
      up_step.append(BatchNormalization())
      up_step.append(Conv2D((2**(3-i))*16, 3, padding='same', kernel_regularizer=l1_l2(0.4, 0.4)))
      up_step.append(BatchNormalization())
      self.decoder.append(up_step)
    self.out_conv = Conv2D(3, 1, kernel_regularizer=l1_l2(0.4, 0.4),activation='softmax')
